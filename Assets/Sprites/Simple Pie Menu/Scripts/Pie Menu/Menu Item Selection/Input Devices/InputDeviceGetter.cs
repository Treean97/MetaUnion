...
namespace SimplePieMenu
{
    public class InputDeviceGetter : MonoBehaviour
    {
        ...
        private void HandleInputDevicePreferences()
        {
            ...
            if (isOldInputSystemEnabled && inputDeviceId == (int)AvailableInputDevices.MouseAndKeyboard_OLD_INPUT_SYSTEM)
            {
                SetInputDevice<MouseAndKeyboard_OLD_INPUT_SYSTEM>();
            }
            else if (isNewInputSystemEnabled && inputDeviceId == (int)AvailableInputDevices.MouseAndKeyboard_NEW_INPUT_SYSTEM)
            {
        ...
        private void SetInputDevice<T>() where T : MonoBehaviour, IInputDevice
        {
            ...
            {
                throw new InvalidOperationException($"Type {typeof(T)} must derive from MonoBehaviour" +
                    $" and implement IInputDevice interface.");
            }
        }
    }
}
